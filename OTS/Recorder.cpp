...
Recorder::Recorder(int width, int height, Platform* platform)
	: width(width), height(height), platform(platform), isRecording(false), totalSize(0)
{
    if (width <= 0 || height <= 0) {
...
void Recorder::getFrame(std::vector<uint32_t>& frame) {
    std::vector<uint32_t>* newFrame = new std::vector<uint32_t>(width*height);
    if (isRecording) addFrame(*newFrame, newFrame->size());
